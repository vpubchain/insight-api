...
BlockController.prototype.checkBlockHash = function(req, res, next) {
  ...
  if (hash.length < 64 || !isHexadecimal(hash)) {
    this.node.services.vpubd.getBlockHeader(parseInt(hash), function(err, info) {
      if (err) {
...
BlockController.prototype.blockHeader = function(req, res, next) {
	...
	self.node.services.vpubd.getBlockHeader(hash, function(err, info) {
		if (err) {
			return self.common.handleErrors(err, res);
		}
...
BlockController.prototype.blockHeaders = function(req, res, next) {
	...
	if(blockIdentifier.length===64){
		...
		var result = self.node.services.vpubd.getBlockHeaders(hash, cb, nbOfBlockToFetch);
	}else{
		var height = blockIdentifier;
		var result = self.node.services.vpubd.getBlockHeaders(height, cb, nbOfBlockToFetch);
	}
...
BlockController.prototype.block = function(req, res, next) {
  ...
  if (blockCached) {
    blockCached.confirmations = self.node.services.vpubd.height - blockCached.height + 1;
...
BlockController.prototype._normalizePrevHash = function(hash) {
  // TODO fix vpubcore to give back null instead of null hash
...
BlockController.prototype.blockIndex = function(req, res) {
  ...
  this.node.services.vpubd.getBlockHeader(parseInt(height), function(err, info) {
    if (err) {
      return self.common.handleErrors(err, res);
    }
    res.jsonp({
...
BlockController.prototype._getBlockSummary = function(hash, moreTimestamp, next) {
  ...
  if (summaryCache) {
    ...
    self.node.services.vpubd.getRawBlock(hash, function(err, blockBuffer) {
      console.log("getRawBlock:"+hash,err);
      if (err) {
        return next(err);
      }

      var br = new vpubcore.encoding.BufferReader(blockBuffer);
      ...
      // would require changes in vpubcore-node.
      var header = vpubcore.BlockHeader.fromBufferReader(br);
      var info = {};
      var txlength = br.readVarintNum();
      info.transactions = [vpubcore.Transaction().fromBufferReader(br)];

      self.node.services.vpubd.getBlockHeader(hash, function(err, blockHeader) {
        if (err) {
          return next(err);
        }
        ...
        var confirmations = self.node.services.vpubd.height - height + 1;
...
BlockController.prototype.list = function(req, res) {
  ...
  //get方式请求
  var url = "http://47.104.97.209:3001/ext/getblockhashbytime/"+gte+"/"+lte;
  ...
  // https.get("https://pl.vpubchain.net/ext/getblockhashbytime/"+gte+"/"+lte,function (res) {  
  //   var datas = [];  
  //   var size = 0;  
  //   console.log("RES:",res);
  ...
  //       var hashes = [];
  //       if(buff){
  //         let hasheStr = iconv.decode(buff, "utf8");
  //         hashes = JSON.parse(hasheStr);
  //       }
  ...
  // }); 
  // self.node.services.vpubd.getBlockHashesByTimestamp(lte, gte, function(err, hashes) {
  //   if(err) {
  //     return self.common.handleErrors(err, res);
  //   }
...
BlockController.prototype.getPreviousBlock = function(prevHash, cb) {
  ...
  if(prevHash===null) cb(null,null);
      ...
      self.node.getBlock(prevHash, function (err, block) {
          ...
          self.node.services.vpubd.getBlockHeader(prevHash, function (err, info) {
              if (err) {
